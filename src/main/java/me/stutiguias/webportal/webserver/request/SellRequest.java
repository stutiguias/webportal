...
public class SellRequest extends HttpResponse {
    ...
    public SellRequest(WebPortal plugin, HttpExchange exchange) {
        super(plugin);
    ...
    public void Cancel(String url,Map param,String sessionId) {
        ...
        ItemMeta meta = auction.getItemStack().getItemMeta();
        int cancelItemDamage = meta != null ? ((Damageable) meta).getDamage() : 0;
        ...
        if(!auctions.isEmpty() && auction.getItemStack().getType() != Material.ENCHANTED_BOOK) {
            ...
            int newAmount = auction.getItemStack().getAmount() + auctions.get(0).getItemStack().getAmount();
            int itemId = auctions.get(0).getId();
